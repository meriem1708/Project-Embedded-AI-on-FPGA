...
static inline fx_t float_to_fx(float x)
{
    float scaled = x * (float)(1 << FX_FRAC_BITS);
...
{
    return (float)x / (float)(1 << FX_FRAC_BITS);
}

/* Multiplication fixe (Qm.n × Qm.n -> Qm.n) */
static inline fx_t fx_mul(fx_t a, fx_t b)
{
    int64_t tmp = (int64_t)a * (int64_t)b;   // produit sur 64 bits
    tmp >>= FX_FRAC_BITS;                   // réalignement de la fraction
...
/* Prototypes FIXED pour les couches */
