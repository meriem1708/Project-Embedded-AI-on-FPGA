    ...
    for (y = 0; y < CONV1_DIM; y++)
      for (x = 0; x < CONV1_DIM; x++) 
		for (z = 0; z < IMG_DEPTH; z++) //{
		  //float w = buffer_float[y][x][z][k];  // valeur lue (format HDF5)

                    // Réordonnancement vers ton tableau interne
                    weight[k][z][y][x] = buffer_float[y][x][z][k];;

                    // Dump pour construire weights.h
                    //printf("CONV1_KERNEL_F[%u][%u][%u][%u] = %.8ff;\n",
                           //(unsigned)k, (unsigned)z, (unsigned)y, (unsigned)x, w);
                   //}
...
void ReadConv1Bias(char *filename, char *datasetname, float *bias) {
  ...
  for (k = 0; k < CONV1_NBOUTPUT; k++) {
    bias[k] = buffer_float[k]; 
     // Ligne pour recuperer les valeurs dans weights.h
        //printf("CONV1_BIAS_F[%u] = %.8ff;\n", (unsigned)k, bias[k]);
  }
  status = H5Dclose (dataset);
  status = H5Fclose (file);
